...
router.get('/:id', async (req, res) => {
  ...
  try {
    const CategoryData = await Category.findByPk(req.params.id, {
      ...
      //include: [{ model: Product, through: Tag, as: 'tag_product' }] //need to ask
    });
...
router.post('/', async (req, res) => {
  ...
  try {
    const categoryData = await Category.create(req.body);  // need to change here
    res.status(200).json(categoryData);
  } catch (err) {
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const categoryData = await Category.update(req.body);  // need to change here
    res.status(200).json(categoryData);
    } catch (err) {
      res.status(400).json(err);
    }
...
router.delete('/:id',  async (req, res) => {
  try {
    ...
    if (!categoryData) {
      res.status(404).json({ message: 'can not found it with this id' });
      return;
    }
    res.status(200).json(categoryData);
  } catch (err) {
